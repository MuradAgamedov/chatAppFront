...
const SettingsForm = () => {
    ...
    const handleAvatarChange = (e) => {
        ...
        if (file) {
            // Сохраняем сам файл для отправки на сервер
            setAvatar(file);

            // Создаем временный URL для предпросмотра
            const previewUrl = URL.createObjectURL(file);
            setAvatarPreview(previewUrl);  // Сохраняем URL для предпросмотра
        }
    };
    ...
    const handleSubmit = async (e) => {
        ...
        // Если файл аватара не выбран, отправляем текущий аватар
        if (avatar) {
            formData.append("avatarUrl", avatar);  // Отправляем сам файл
        } else {
            formData.append("avatarUrl", currentAvatarUrl);  // Отправляем текущий URL
        }
    ...
    return (
        <div style={{ height: '100vh' }} className="w-full md:w-2/3 border flex flex-col p-4 bg-white shadow-lg">
